...
const EmploymentDetails = ({
  ...
  const HandleSubmit = () => {
    ...
    // if(validateWidget(validateValues) === true){
      
    // getTodo();
    // console.log(values);
    // message.success("Processing Complete and Fetching Todo");

    // onBoardingAction();
    // showModalAction();
    // }
    // else if(validateWidget(validateValues) === false) {
    //   message.error("Kindly fill all the inputs field");
      
    validateWidget(validateValues)
...
export default connect(null,{onBoardingAction, showModalAction}) (EmploymentDetails);
